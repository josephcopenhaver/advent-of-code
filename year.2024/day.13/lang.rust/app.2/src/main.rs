...
fn parse_line(s: &str, offset: i64, sep: &str) -> Result<(i64, i64), Box<dyn Error>> {
    ...
    Ok((offset + x.parse::<i64>()?, offset + y.parse::<i64>()?))
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for v in INPUT.split("\n\n") {
        ...
        let a = parse_line(a, 0, "+")?;
        let b = parse_line(b, 0, "+")?;
        let p = parse_line(p, PRIZE_OFFSET, "=")?;
