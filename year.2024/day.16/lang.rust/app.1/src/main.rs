...
impl Bitmap {
    ...
    //     let mut dst = Bitmap::new(self.0.len() * 8);
    ...
    // fn clone_into(&self, dst: &mut Bitmap) {
    //     for (i, v) in self.m.iter().cloned().enumerate() {
    //         dst.0[i] = v;
    //     }
    // }
...
// note that this solution assumes there is never a path behind the reindeer at start
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let idx_to_point = |i: usize| -> (i32, i32) {
        let x = i % w;
        let y = i / w;
        (x as i32, y as i32)
    };
    ...
    let (num_verts, edges, start_idx, end_idx) = {
        ...
        for idx in vertex_bm.iter_set() {
            ...
            for enter_dir in dbuf.iter().cloned() {
                ...
                let mut nidx = {
                    // TODO: make a point that supports a translate() operation to make a new point
                    let d = Dir::translations()[enter_dir as usize];
                ...
                while !dead_end && !vertex_bm.is_set(nidx as usize) {
                    ...
                    for (i, d) in Dir::translations().iter().cloned().enumerate() {
                        ...
                        if d != nd {
                            score += if d.rotate_180() == nd { 2000 } else { 1000 };
                            nd = d;
                        }
                        nidx = idx as i32;
                ...
                for v in dbuf.iter().cloned() {
                    ...
                    score += if prev_dir == enter_dir { 0 } else { 1000 };
    ...
    let sum = {
        ...
        let mut origins = HashSet::<i32>::with_capacity(num_verts); // TODO: make this a hashmap so pairs can be traversed rather than calling a hash operation each start of the origin iteration (duplicate visited's value data)
        let mut exhausted_origins = Vec::<i32>::with_capacity(num_verts);
        visited.insert(start_idx, (0, Dir::Right));
        origins.insert(start_idx);
        while !visited.contains_key(&end_idx) {
            ...
            for origin in origins.iter().cloned() {
                let (prev_score, d) = if let Some(v) = visited.get(&origin).cloned() {
                    v
                } else {
                    panic!("failed to get data of a visited node");
                };

                let prev_state = Reindeer::new(origin, d);
                ...
                loop {
                    ...
                    if prev_score + score_inc < old_next.0 {
                        next = Some((prev_score + score_inc, end_state));
                    }
            ...
            origins.insert(end_state.idx);
            visited.insert(end_state.idx, (end_score, end_state.dir));
        }
